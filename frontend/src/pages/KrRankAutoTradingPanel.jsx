      ...
      <section className="panel section">
        <div className="panel-heading">
          <div>
            ...
            <p>오전 9시부터 10분간, 점심 진입을 켜면 11시 20분부터 10분간 한국주식 등락률 상위 랭킹을 반복 관찰합니다. 9시 10분·11시 30분부터 5분 동안 최근 흐름을 갱신해 보고, 원본 랭킹 상위 10위 안에서 꾸준히 보인 종목만 두 번 확인한 뒤 주문합니다.</p>
          </div>
        </div>
        <ul className="kr-rank-rule-list">
          ...
          <li>오전은 실거래 복기에서 상대적으로 유리했던 등락률 15% 이상 20% 미만, 점심은 되돌림 위험이 낮았던 15% 미만 종목만 봅니다. 우선주와 ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 매수 대상에서 뺍니다.</li>
          <li>최근 최대 12개 랭킹 스냅샷에서 절반 이상 등장했고 현재도 원본 랭킹 상위 10위에 남은 종목만 봅니다. 첫 판단에서 모두 탈락해도 5분 안에는 계속 관찰하지만, 그 뒤에는 새 후보를 쫓지 않고 해당 진입을 쉬어 갑니다.</li>
          <li>당일 분봉으로 단기 흐름을 한 번 더 봅니다 — 가장 최근 완성봉에 실제 체결이 있음(거래량 0 제외), 관찰 구간 거래대금 3억원 이상, 최근 3분 거래대금 3천만원 이상, 시가 위, VWAP보다 0.1% 이상 위, VWAP 대비 과열 아님(오전 8%, 점심 6% 이내), 점심은 최근 VWAP 기준 과열 아님, 최근 3분 4%·8분 7% 이상 수직 급등 아님, 최근 완성봉 2개 VWAP 상회, 거래량 유지, 거래량 동반 장대 음봉 없음, 최근 고점 대비 1.2% 이상 밀리지 않음, 직전 고점의 99.5% 이상 유지. 종합 후보 5개 안에서 통과 후보를 점수로 고르고, 모두 떨어지면 그 구간 매수는 건너뜁니다.</li>
          <li>후보가 한 번 통과해도 바로 주문하지 않고 다음 평가에서 랭킹·분봉·가격을 다시 확인합니다. 매수는 현재가 근처(현재가 +0.4%) 지정가로 보내며, 신호가보다 0.7% 넘게 오르면 추격, 0.7% 넘게 내리면 모멘텀 무효로 봅니다. 명시적 주문 거절은 최신 신호를 다시 확인한 뒤 제한 재시도하고, 90초간 전혀 체결되지 않은 주문은 취소 완료를 확인한 뒤에만 재호가합니다.</li>
          <li>계좌에 같은 종목을 이미 보유하고 있으면 수동 보유분과 섞이지 않도록 새 실매수를 하지 않습니다. 매도도 이 전략의 매수 체결 수량에서 이미 판 수량을 뺀 만큼만 주문합니다.</li>
          <li>손실 청산 후에도 전략은 계속 실행되며, 오전 손실만으로 점심 수익 기회를 자동 차단하지 않습니다. 실현손익 확인이 늦은 청산은 실제 체결가가 없으면 이익으로 넘겨짚지 않고 손익 미확정으로 집계합니다. 이미 접수된 주문 확인과 보유 종목 청산은 항상 계속합니다.</li>
          ...
          <li>설정 손절이 5%보다 엄격하면 초기 흔들기를 잠깐 확인할 수 있지만, 손실이 -5%에 닿으면 유예 없이 시장가 청산을 시작합니다. 급격한 갭 하락이나 거래정지에서는 -5% 가격 체결을 보장할 수 없습니다.</li>
