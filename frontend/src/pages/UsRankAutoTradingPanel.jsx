...
export function UsRankAutoTradingPanel({ liveOrderEnabled, periodReturns, onPeriodReturnsRefresh }) {
  ...
  async function runReplay(order) {
    if (!selected || !order?.tradeId) return;
    setBusy(`replay-${order.tradeId}`);
    setError('');
    setReplay(null);
    try {
      setReplay(await replayUsRankTrade(selected.id, order.tradeId));
    } catch (err) {
    ...
    <>
      ...
      <section className="panel section">
        ...
        <ul className="kr-rank-rule-list">
          ...
          <li>진입 후보: 등락률 +50% 미만, 가격 5 USD 이상, 거래량 1,000만 주 이상, 거래대금(가격×거래량) 5천만 USD 이상인 상위 종목만 봅니다(초저가·과열·저유동성 종목 배제).</li>
          ...
          <li>주문번호 없이 명시적으로 거절된 매수는 다음 평가에서 같은 종목이 최신 상위 3개와 분봉 조건을 다시 통과하고, 최초 선택가에서 ±2% 안에 있을 때만 제한적으로 다시 시도합니다. 응답이 끊겨 접수 여부를 모르면 이중매수를 막기 위해 재전송하지 않습니다.</li>
          ...
          <li>설정 손절이 더 느슨해도 -5%부터 청산을 시작하며, 미체결 방어 매도는 다음 30초 평가에서 취소·재호가합니다. 급격한 갭에서는 실제 체결 손실이 -5%를 넘을 수 있습니다.</li>
          <li>손실이 커지고 분봉 흐름이 무너지면 고정 손절을 기다리지 않고 빠른 손절로 먼저 정리합니다. 한 봉의 일시적 출렁임에 팔지 않도록 VWAP 아래 2봉 연속·직전 봉 회복 없음·최근 지지(스윙 저점) 이탈을 함께 확인하고, 변동성(ATR)이 클수록 더 큰 하락에서만 발동합니다(미국장은 최소 손실 3% 바닥). 진행 중인 봉은 빼고 완성봉으로만 판단합니다.</li>
                ...
                <tr key={order.tradeId}>
                  ...
                  <td>
                    <button
                      type="button"
                      className="ghost sm"
                      disabled={!order.buyPrice || replayBusyId === String(order.tradeId)}
                      onClick={() => onReplay?.(order)}
                    >
                      {replayBusyId === String(order.tradeId) ? '복기 중…' : '거래 복기'}
                    </button>
                  </td>
                </tr>
              );
