...
export function HoldingPanel({ holding }) {
  ...
  return (
    <section className="panel section">
      ...
      <div className="metric-grid">
        ...
        <Metric label="평균단가" value={`${formatMoney(holding.averagePrice)}원`} hint="여태 매수한 가격의 평균" />
        <Metric label="투입금" value={`${formatMoney(holding.investedAmount)}원`} hint="가상으로 투입된 누적 금액" />
        <Metric label="잔여 예산" value={`${formatMoney(holding.remainingBudget)}원`} hint="앞으로 더 매수 가능한 금액" />
        <Metric
          label="실현 손익"
          value={`${formatSigned(realized)}원`}
          hint="매도 체결로 확정된 손익"
