...
- 보유 중 최근 분봉이 VWAP·시가·최근 고점·거래량 기준으로 빠르게 무너지면 `ENTRY_FAILED` 진입 실패 매도로 전량 청산을 시도한다.
...
- 목표가 주문이 미체결인 상태에서 손절·진입 실패·강제 청산·누적 목표 종료가 필요하면 기존 목표가 주문을 먼저 취소한다. 취소 성공을 확인하지 못하면 신규 매도 주문을 만들지 않고 판단 로그에 충돌 방지 사유를 남긴다.
