...
사용자별 KIS 자격증명을 사용해 단일 종목 자동매매 전략(`LAOR_INFINITE_V2`)을 RUNNING 상태로 운용한다. 스케줄러가 주기 평가, 안전 검증, 모의 주문, 판단 로그·포지션 스냅샷 기록을 담당한다. 현재 production은 `LAOR_LIVE_ORDER_ENABLED=false`로 고정해 라오어 신규 live 주문을 금지한다. `ACCEPTED`와 `FILLED` 분리, timeout `UNKNOWN`, 전략 관리 수량과 외부 동일종목 잔고 격리를 랭킹 전략 수준으로 보강하기 전에는 사용자 토글이 켜져도 `DRY_RUN`만 저장한다.
...
- `ENABLE_LIVE_ORDER=false`는 사용자 설정보다 우선하는 전역 kill switch다. KIS 거래 서비스의 BUY·SELL·취소 최종 경계에서 모든 쓰기를 차단한다. 랭킹 전략의 기존 live 주문·포지션은 조회와 체결 상태 동기화만 계속하며 DRY_RUN 상태로 변환하지 않는다.
- `LAOR_LIVE_ORDER_ENABLED=false`는 라오어에만 적용되는 별도 release gate다. 이 값이 false이면 라오어는 사용자 실주문 설정과 무관하게 `liveOrderEnabled=false`로 평가하고 KIS 주문·취소 POST를 호출하지 않는다.
- `KR_RANK_LIVE_ENTRY_RETRY_ENABLED=false`이면 KR의 첫 live 판단이 후보 없음으로 끝난 뒤 5분 동안 실제 top 30 랭킹만 shadow로 저장하고, rolling 재탐색 결과로 신규 BUY를 만들지 않는다. production은 이 값을 `true`로 두어 진입 시작 후 5분 동안 재탐색하고, 후보의 다음 tick 재확인과 기존 주문 안전 검사를 모두 통과한 경우에만 신규 BUY를 만든다.
- `KR_RANK_CORE_FALLBACK_STRATEGY_IDS`는 오전 strict 후보가 없을 때 core fallback을 허용할 KR 전략 ID allowlist다. production은 test3 전략 4만 포함하고 점심에는 적용하지 않는다. fallback도 오전 15~20%, 원본 top 10, 최신 잔존, 다음 tick ±0.7%, 매수가능금액·미체결·중복·잔고 검사를 유지한다.
...
자동매매 도메인은 라오어 무한매수법 외에 한국 국장 상승률 랭킹 전략을 두 번째 독립 전략 종류로 함께 운용한다. 두 전략 종류는 각자의 테이블(`kr_rank_*`)·엔진(`krRankStrategyEngine`/`krRankService`)·평가 경로를 가지며, 실주문 실행 설정(`user_trading_settings.live_order_enabled`)·스케줄러(`autoTradingScheduler`)·KIS 연동(`kisAuthService`/`kisTradingService`)을 공유한다. 한국 랭킹 전략은 오전 09:00~09:10과 선택적 점심 11:20~11:30 동안 상승률 랭킹을 반복 관찰한다. 오전 09:10·점심 11:30 첫 live 판단이 후보 없음이면 `KR_RANK_LIVE_ENTRY_RETRY_ENABLED=false`에서 해당 entry를 `NO_CANDIDATE`로 종결하되 이후 5분 실제 top 30 랭킹은 shadow 관찰로 계속 저장한다. production은 이 flag를 true로 두고 최근 최대 12개 스냅샷과 현재 랭킹을 갱신해 5분 동안 후보를 다시 찾는다. 각 스냅샷은 원본 랭킹 상위 10위를 먼저 제한하며, 관찰이 충분하면 절반 이상 등장하고 최신 랭킹에도 남은 종목만 인정한다. 후보가 통과해도 다음 tick에서 원본 순위·분봉·가격을 재확인하며, 최초 선택가나 최신 신호가 대비 ±0.7% 밖으로 움직이면 모멘텀 무효 또는 추격으로 종결한다. 오전은 +15% 이상 +20% 미만, 점심은 +15% 미만 종목만 허용하고 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품을 배제한다. 실주문 손실 뒤에도 `RUNNING` 상태와 다음 진입 평가를 유지하며, 오전 손실 1회만으로 점심 진입을 차단하지 않는다. 같은 거래일의 손실 또는 손익 미확정 청산이 2회 누적되면 그날 추가 신규 진입만 제한하고 다음 거래일에 자동 재개한다. KIS 실현손익이 늦으면 BUY·SELL 실제 체결가로만 보완하며, 주문 결정가는 체결가로 추정하지 않는다. 같은 진입의 분할 SELL은 한 번으로 집계하고, 체결 동기화와 기존 보유 청산은 계속한다. 신규 live BUY 직전 동일 종목 잔고 0을 요구하고, 이후 BUY·SELL 수량도 해당 전략 주문의 누적 체결수량으로 제한한다. 설정 손절이 더 느슨해도 -5% 손실부터 초기 흔들기 유예 없이 시장가 청산을 시작한다. 상세 동작은 `kr-rank-auto-trading` 스펙을 참고한다. 평일·장 운영 시간(09:00~15:30 KST)을 통과해도, 공휴일에는 KIS 국내휴장일조회(TR `CTCA0903R`, `opnd_yn`)로 개장일이 아님을 확인하면 주문하지 않고 평가만 건너뛴다(휴장 판정은 날짜별 1회 조회 후 캐시).
...
- 매수는 평가 시점 미국 종목 매수가능금액 전액을 사용한다. 주문 수량은 정수 1주 단위다. KIS가 주문번호 없이 거절을 명시하면 다음 평가에서 동일 종목이 최신 top 3와 분봉 필터를 다시 통과하고 최초 선택가 대비 ±2% 안에 있을 때만 제한 재시도한다. timeout처럼 접수 여부가 모호한 `UNKNOWN`은 자동 재전송하지 않는다.
- 보유 중에는 새 종목을 사지 않고 익절(기본 +2%), 사용자 설정 손절과 -5% 하드 방어선 중 더 엄격한 기준, 강제 청산(KST 기본 04:30)을 평가한다.
- 매수 체결이 확인되면 평균 체결가 기준 목표 수익 지정가 매도를 즉시 제출한다. 실주문 OFF에서는 실제 KIS 주문 없이 목표가 주문 예정 기록만 남긴다.
- 보유 중 손실이 일정 수준 이상이고 최근 분봉 흐름이 구조적으로 무너지면 `ENTRY_FAILED` 빠른 손절 매도로 전량 청산을 시도한다. 모멘텀주의 흔들기(아래꼬리)를 추세 붕괴로 오판하지 않도록 세 가지 방어를 둔다(`evaluateFastStopLoss`/`evaluateEntryFailure`): **(1) 확인** — 마지막 2개 완성봉이 VWAP 아래에 연속으로 머물고 직전 봉 종가를 회복(반등)하지 않을 때만 붕괴로 본다(한 봉 아래꼬리로는 발동하지 않음). 라이브 평가는 진행 중(미완성) 봉을 한 봉 떼고 본다. **(2) 구조 기준** — "절대 고점 대비 N% 되돌림"이 아니라 VWAP 지속 이탈 + 최근 스윙 저점(지지) 이탈 또는 거래량 동반 장대 음봉으로 판정한다. **(3) 변동성 적응형 트리거** — 손실 하한을 고정값과 최근 ATR(평균 봉 범위)×2.5 중 큰 값으로 둔다(한국장 최소 2%, 미국장 최소 3% 바닥). 출렁임이 큰 종목은 더 큰 붕괴가 있어야 발동해 노이즈 손절을 줄인다. 매수 후 20분이 지나면 빠른 손절을 적용하지 않고 고정 손절·목표가에 맡긴다.
- 익절 조건에 닿으면 이미 걸어 둔 목표가 주문 체결을 확인하고, 체결되면 랭킹 순위와 관계없이 전량 매도 확정 후 다음 tick에서 새 랭킹 후보로 다음 매매 사이클을 시작할 수 있다.
- **매도 체결 안전장치(중요)**: KIS 주문 접수(`ACCEPTED`)는 체결이 아니다. 실주문 매도는 접수만으로 청산을 확정하지 않고, 다음 평가에서 KIS 체결조회(`inquire-ccnl`, TR `TTTS3035R`)/잔고로 체결을 확인한 뒤에만 `CLOSED`·보유 해제·당일 잠금·실현손익을 확정한다(체결가는 판단 시점 현재가가 아닌 실제 평균 체결가로 기록). 미체결 매도가 `SELL_STALE_LIMIT_MS`(25초)를 넘기면 다음 30초 평가에서 취소하고 더 공격적인 가격(`aggressiveSellPrice`: 현재가보다 0.5%~최대 5% 아래 지정가, 재호가일수록 깊게)으로 재호가해 손절이 명목상으로만 남지 않게 한다. 실주문 OFF(`DRY_RUN`)는 확인할 체결이 없어 즉시 청산을 시뮬레이션한다.
