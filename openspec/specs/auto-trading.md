...
자동매매 도메인은 라오어 무한매수법 외에 한국 국장 상승률 랭킹 전략을 두 번째 독립 전략 종류로 함께 운용한다. 두 전략 종류는 각자의 테이블(`kr_rank_*`)·엔진(`krRankStrategyEngine`/`krRankService`)·평가 경로를 가지며, 실주문 실행 설정(`user_trading_settings.live_order_enabled`)·스케줄러(`autoTradingScheduler`)·KIS 연동(`kisAuthService`/`kisTradingService`)을 공유한다. 라오어 전략의 평가 사이클·상태 머신·기록은 이 전략 추가로 변경되지 않는다. 한국 랭킹 전략은 오전 09:00~09:10과 선택적 점심 11:20~11:30 동안 상승률 랭킹을 반복 관찰하고, 오전 09:10·점심 11:30 진입에서 관찰 스냅샷과 현재 랭킹을 종합해 후보를 고른다. 오전은 +21%, 점심은 +15% 이상 종목을 제외하고, 관찰이 충분한데 반복 출현 후보가 없으면 해당 진입을 건너뛴다. 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 배제한다. 분봉 필터는 **가장 최근 완성봉 거래량 > 0**(거래가 마른 유령 분봉을 신호로 쓰지 않음), 관찰 구간 거래대금 3억원 이상, 최근 3분 거래대금 3천만원 이상, 시가·VWAP 상회, VWAP 과열 상한(오전 8%, 점심 6%), 점심 최근 VWAP 과열 상한(4%), 최근 3분 4%·8분 7% 이상 수직 급등 차단, 거래량 유지, 장대 음봉 부재, 고점 유지 조건을 함께 본다. 매수는 시장가가 아니라 **현재가 + 0.4% 캡 지정가**로 보내 추격 슬리피지를 막는다(정상 호가는 즉시 체결, 순간 갭 호가는 미체결로 꼭대기 회피). 또한 실행 직전 실시간 현재가가 분봉 필터가 승인한 신호가보다 +0.7% 넘게 올라 있으면 추격으로 보고 그 평가의 매수를 보류한다(다음 tick에 신호가 안정되면 재평가). 매수 직후 고정 손절선에 닿아도 손실이 10% 미만이고 완성봉 기준 칼낙·분출 매도·구조 붕괴가 확인되지 않으면 초기 흔들기로 보고 최대 10분까지 손절을 유예한다. 상세 동작은 `kr-rank-auto-trading` 스펙을 참고한다. 평일·장 운영 시간(09:00~15:30 KST)을 통과해도, 공휴일에는 KIS 국내휴장일조회(TR `CTCA0903R`, `opnd_yn`)로 개장일이 아님을 확인하면 주문하지 않고 평가만 건너뛴다(휴장 판정은 날짜별 1회 조회 후 캐시).
