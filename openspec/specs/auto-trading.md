...
자동매매 도메인은 라오어 무한매수법 외에 한국 국장 상승률 랭킹 전략을 두 번째 독립 전략 종류로 함께 운용한다. 두 전략 종류는 각자의 테이블(`kr_rank_*`)·엔진(`krRankStrategyEngine`/`krRankService`)·평가 경로를 가지며, 실주문 실행 설정(`user_trading_settings.live_order_enabled`)·스케줄러(`autoTradingScheduler`)·KIS 연동(`kisAuthService`/`kisTradingService`)을 공유한다. 라오어 전략의 평가 사이클·상태 머신·기록은 이 전략 추가로 변경되지 않는다. 한국 랭킹 전략은 오전 09:00~09:10과 선택적 점심 11:20~11:30 동안 상승률 랭킹을 반복 관찰하고, 오전 09:10·점심 11:30의 첫 유효 평가에서 후보를 고른다. 각 스냅샷은 원본 랭킹 상위 10위를 먼저 제한하며, 관찰이 충분하면 절반 이상 등장하고 최신 랭킹에도 남은 종목만 인정한다. 후보가 모두 거절되면 해당 구간을 종결하고 늦은 후보를 반복 탐색하지 않는다. 후보가 통과해도 다음 tick에서 원본 순위·분봉·가격을 재확인하며, 최초 선택가나 최신 신호가 대비 ±0.7% 밖으로 움직이면 모멘텀 무효 또는 추격으로 종결한다. 오전은 +21%, 점심은 +15% 이상 종목과 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품을 배제한다. 실주문 손실 또는 손익 미확정 청산이 발생한 날은 남은 진입을 막고, 최근 청산 2건이 연속 위험 상태이면 `RUNNING` 상태를 유지한 채 신규 진입만 잠근다. KIS 실현손익이 늦으면 BUY·SELL 체결가로 보완하고, 같은 진입의 분할 SELL은 한 번으로 집계한다. 체결 동기화와 기존 보유 청산은 계속하며, 계좌 확인 후 명시적 stop→start로 잠금을 해제한다. 신규 live BUY 직전 동일 종목 잔고 0을 요구하고, 이후 BUY·SELL 수량도 해당 전략 주문의 누적 체결수량으로 제한한다. 매수 직후 고정 손절선에 닿아도 손실이 10% 미만이고 완성봉 기준 칼낙·분출 매도·구조 붕괴가 확인되지 않으면 초기 흔들기로 보고 최대 10분까지 손절을 유예한다. 상세 동작은 `kr-rank-auto-trading` 스펙을 참고한다. 평일·장 운영 시간(09:00~15:30 KST)을 통과해도, 공휴일에는 KIS 국내휴장일조회(TR `CTCA0903R`, `opnd_yn`)로 개장일이 아님을 확인하면 주문하지 않고 평가만 건너뛴다(휴장 판정은 날짜별 1회 조회 후 캐시).
