...
- `auto_trading_strategies` — 라오어 자동매매 전략. status (`CREATED`/`RUNNING`/`STOPPED`/`ERROR`), 종목·시장·통화·`exchange`(거래소 코드, `0025`), 총 예산, 분할 회차, 회차당 매수 금액, `cycle_budget`, 목표 수익률, 현재 회차, 현재 회차 거래일(`round_trade_date`, `0026`), (레거시) 1회·일일 주문 한도, `big_buy_premium_rate`, 레거시 pending 예산 컬럼, started/stopped/last_evaluated/last_order_at, last_decision, last_error_message.
- `auto_trading_position_snapshots` — 평가 시점 보유 수량·평단·현재가·평가금·미실현·현금 + `decision` (그 시점의 판단).
- `auto_trading_orders` — 주문 라이프사이클. `idempotency_key` 인덱스로 조회하되 UNIQUE 제약은 없다(`0026`에서 제거). 같은 멱등키의 실패 주문을 제한 재시도할 수 있게 하기 위함이다. `exchange`(거래소 코드, `0025`).
...
## 한국 국장 상승률 랭킹 자동매매 (`0024`, `0026~0028`, `0031~0032`)
...
## 미국장 상승률 랭킹 자동매매 (`0029~0030`)

라오어 자동매매(`auto_trading_*`)와 한국 랭킹(`kr_rank_*`)을 변경하지 않는 별도 테이블 세트. 실주문 실행 설정(`user_trading_settings`)과 KIS credential만 공유한다.

- `us_rank_strategies` — 미국장 상승률 랭킹 전략(`US_RANK_MOMENTUM`). status, 자동 예산 여부, 고정 매수 금액(USD, 현재 service는 생성 입력을 자동 예산 모드로 정규화해 0 저장), 익절률, 손절률, 강제 청산 시각(KST), 거래소(`ALL`/`NAS`/`NYS`/`AMS`), 통화(`USD`), 누적 목표 수익률·기준 자본·완료 시각(`0030`), 현재 보유 종목, 당일 신규 매수 잠금(`day_locked_out`)과 잠금 사유, soft delete(`deleted_at`). 미국장은 `0030`에서 전략별 등락률 상한 컬럼을 제거했지만, 현재 후보 필터는 코드 상수(`US_MAX_FLUCTUATION_RATE=0.50`)로 과열 급등 종목을 제외한다.
