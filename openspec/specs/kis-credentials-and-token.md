...
| Method | Path | 설명 |
| --- | --- | --- |
| GET | `/api/settings/kis` | 현재 저장된 KIS 설정 메타 조회 (`configured`, `status`, `appKeyMasked`, `accountConfigured`, `lastTokenIssuedAt`, `lastTokenErrorMessage`). App Secret/access token/계좌번호 원문은 절대 반환하지 않음. |
...
1. KIS 호출 직전에 `kisAuthService.getAuthContext(userId)` 또는 주문/계좌 경로의 `kisTokenManager.getValidAccessToken(userId)` 호출.
2. 저장된 token이 유효하면 그대로 사용. 시장 데이터용 `kisAuthService`는 만료 60초 전, 주문/계좌용 `kisTokenManager`는 만료 5분 전부터 재발급 대상으로 본다. 만료 임박 또는 없음이면 저장된 App Key/Secret으로 KIS `/oauth2/tokenP`에 재요청.
...
- 계좌번호는 암호화 저장하며 KIS 설정 조회 응답에는 계좌 설정 여부(`accountConfigured`)만 노출한다. 계좌번호 원문이나 마스킹 문자열은 반환하지 않는다.
