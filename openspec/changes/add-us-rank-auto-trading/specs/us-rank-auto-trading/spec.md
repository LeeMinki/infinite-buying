...
미국 정규장 시간에 보유 종목이 없고 새 매매 사이클을 시작할 수 있는 상태이면, 시스템은 KIS 해외주식 상승률 상위 랭킹을 조회해 첫 유효 종목을 매수 대상으로 선택해야 한다(MUST). 미국장은 한국장처럼 가격제한폭이 없으므로 별도 상승률 상한 필터를 두지 않는다. 단, 현재가 1 USD 미만 또는 거래량 1,000만 주 미만 종목은 제외해야 한다(MUST). 선택된 종목이 없으면 매수하지 않고 판단 기록을 남긴다. 매수는 현재가 지정가 1주 단위 정수 주문이며, KIS 매수가능금액 전액으로 매수 가능 수량을 계산한다.

#### Scenario: 첫 유효 종목 선택
- **WHEN** 평가 시점 랭킹 1위 등락률 +35%, 2위 +18%, 3위 +12%이고 1위 종목의 종목코드·현재가·거래량이 유효함
- **THEN** 1위 종목이 매수 대상으로 선택되어야 한다

#### Scenario: 선택 가능 종목 없음
- **WHEN** 모든 랭킹 종목의 종목코드가 없거나 현재가가 1 USD 미만이거나 거래량이 1,000만 주 미만이거나 등락률 파싱에 실패함
