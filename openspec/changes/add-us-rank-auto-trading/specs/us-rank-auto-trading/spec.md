...
시스템은 매 평가에서 현재 시각이 미국 정규장(NYSE/NASDAQ 09:30~16:00 ET) 안인지 자동으로 판정해야 한다(MUST). 미국 서머타임으로 인한 시차 변경은 OS의 IANA 시간대 데이터(`America/New_York`)에 위임해 코드에서 별도 분기를 두지 않아야 한다(MUST). 정규장 외에는 매수·매도 평가를 하지 않고 idle SKIP으로 종료한다.
...
### Requirement: 1분 간격 전용 스케줄러

시스템은 미국장 랭킹 전략 전용 1분 간격 스케줄러를 운용해야 한다(MUST). 이 스케줄러는 라오어 10분·한국 랭킹 1분 스케줄러와 독립된 타이머로 동작하며, 한 종류의 평가 오류가 다른 종류의 평가를 중단시키지 않아야 한다.

#### Scenario: 1분 간격 평가
- **WHEN** 미국 정규장 시간에 RUNNING 전략이 있음
- **THEN** 1분 이내의 다음 평가 tick에서 평가가 실행되어야 한다
...
미국 정규장 시간에 보유 종목이 없고 새 매매 사이클을 시작할 수 있는 상태이면, 시스템은 KIS 해외주식 상승률 상위 랭킹을 조회해 첫 유효 종목을 매수 대상으로 선택해야 한다(MUST). 미국장은 한국장처럼 가격제한폭이 없으므로 별도 상승률 상한 필터를 두지 않는다. 선택된 종목이 없으면 매수하지 않고 판단 기록을 남긴다. 매수는 현재가 지정가 1주 단위 정수 주문이며, 매수 금액 한도와 KIS 매수가능금액 중 작은 값으로 매수 가능 수량을 계산한다.

#### Scenario: 첫 유효 종목 선택
- **WHEN** 평가 시점 랭킹 1위 등락률 +35%, 2위 +18%, 3위 +12%이고 1위 종목의 종목코드·현재가가 유효함
- **THEN** 1위 종목이 매수 대상으로 선택되어야 한다

#### Scenario: 선택 가능 종목 없음
- **WHEN** 모든 랭킹 종목의 종목코드가 없거나 현재가가 0 이하이거나 등락률 파싱에 실패함
...
### Requirement: 매수 금액 — 자동 예산 + 고정 USD 옵션

시스템은 전략 생성 시 사용자가 자동 예산(`auto_budget_enabled`) 또는 고정 USD 매수 금액(`fixed_buy_usd_amount`)을 선택할 수 있어야 한다(MUST). 자동 예산이 켜져 있으면 평가 시점의 KIS USD 매수가능금액을 그대로 한 종목에 투입한다. 고정 USD 모드면 `min(fixed_buy_usd_amount, cashAvailable)`을 한도로 사용한다. 자동 예산이 켜져 있으면 `fixed_buy_usd_amount`는 무시하고 0으로 저장한다. 자동 예산이 꺼져 있고 `fixed_buy_usd_amount`가 0 이하이면 전략 생성을 거절한다.

#### Scenario: 자동 예산 모드
- **WHEN** `auto_budget_enabled=true` 전략, 매수가능금액 5,000 USD, 종목 현재가 100 USD
- **THEN** 50주 매수가 계산되어야 한다

#### Scenario: 자동 예산 + 매도 후 재매수
- **WHEN** 자동 예산 전략에서 100주를 사 +2% 익절 매도 → 그 직후 평가
- **THEN** 매도 결제잔액을 포함한 최신 매수가능금액으로 새 매매 사이클을 시작해야 한다(사용자 개입 없음)

#### Scenario: 고정 USD 모드
- **WHEN** `auto_budget_enabled=false`, `fixed_buy_usd_amount=2,000`, 매수가능금액 5,000 USD, 현재가 50 USD
- **THEN** 2,000 USD 한도로 40주 매수가 계산되어야 한다

#### Scenario: 고정 모드 0 거절
- **WHEN** `auto_budget_enabled=false`, `fixed_buy_usd_amount=0`으로 전략 생성
- **THEN** 거절해야 한다
...
시스템은 보유분의 수익률이 익절 기준(`target_profit_rate`, 기본 +2%) 이상이면 전량 매도해야 한다(MUST). 단, 익절 기준에 닿았더라도 보유 종목이 현재 상승률 랭킹의 첫 유효 종목이면 매도하지 않고 보유를 유지해야 한다(MUST). 같은 종목을 팔고 바로 다시 사는 불필요한 왕복을 막기 위한 규칙이다. 손실률이 손절 기준(`stop_loss_rate`, 기본 -5%) 이상이면 전량 매도하고 `day_locked_out=true`로 설정해 그날 신규 매수를 정지해야 한다(MUST). 현재 KST 시각이 강제 청산 시각(`force_close_kst`, 기본 04:30) 이상이면 익절·손절 미도달이어도 전량 매도하고 `day_locked_out=true`로 설정해야 한다(MUST). 매도는 현재가 지정가 주문으로 만든다. 같은 평가에서 여러 조건이 동시에 만족하면 누적 목표 달성, 손절, 익절, 강제 청산 순으로 우선 적용한다.

#### Scenario: 익절 매도
- **WHEN** 보유분의 수익률이 익절 기준 이상이고 보유 종목이 현재 상승률 랭킹의 첫 유효 종목이 아님
- **THEN** 전량 매도 주문이 만들어져야 한다 (`sell_reason=TARGET`)

#### Scenario: 익절 도달이지만 보유 종목이 랭킹 1위
- **WHEN** 보유분의 수익률이 익절 기준 이상이고 보유 종목이 현재 상승률 랭킹의 첫 유효 종목임
- **THEN** 매도 주문을 만들지 않고 HOLD 판단을 기록해야 한다
...
시스템은 매 매수·매도·오류 평가, 사용자 수동 평가를 `us_rank_decision_logs`에 기록해야 한다(MUST). 매 주문은 `us_rank_orders`에 라이프사이클(`DECIDED`/`DRY_RUN`/`REQUESTED`/`ACCEPTED`/`FILLED`/`REJECTED`/`FAILED`/`CANCELED`/`UNKNOWN`)을 저장한다. 매 매매 사이클은 `us_rank_trades`에 `trade_seq`·종목·시작/종료 시각·진입가·청산가·청산 사유(`TARGET`/`STOP_LOSS`/`FORCE_CLOSE`/`CYCLE_COMPLETE`)·수익률을 저장한다.
