...
- DST(미국 서머타임) 자동 감지로 ET 10:00~16:00 평가 시간 판정.
...
미국장 랭킹 전략은 ET 10:00~16:00에만 평가한다. ET와 KST 시차는 표준시 14시간, DST 13시간이다.

| 시기 | ET 10:00~16:00 | KST |
|---|---|---|
| 표준시 (11월 첫째 일 ~ 3월 둘째 일) | UTC-5 | 00:00~06:00 (익일) |
| 서머타임 (3월 둘째 일 ~ 11월 첫째 일) | UTC-4 | 23:00~05:00 |

**결정**: `isUsRegularSession(now)`는 미국 동부 시각으로 변환해 `10:00 <= time < 16:00 && weekday`인지 판정한다. KST/UTC 직접 계산 대신 `now.toLocaleString('en-US', { timeZone: 'America/New_York' })`로 ET를 얻어 분 단위로 비교한다. DST는 OS의 IANA tz 데이터가 처리하므로 우리 코드에 DST 분기를 두지 않는다.
...
**결정**: 미국장 랭킹 전략은 고정 매수 금액 옵션 없이 항상 매수가능금액 전액을 기준으로 계산한다.
- 평가 시점 KIS 매수가능금액(USD, `frcr_ord_psbl_amt1`)을 그대로 한 종목에 투입.
- 기존 요청에 `fixed_buy_usd_amount`가 들어와도 미국장 랭킹 전략의 매수 수량 계산에는 사용하지 않는다.

미국주식은 KIS 표준 주문이 정수 1주 단위라 1주 단위로 매수한다(소수점 매매 제외). 매수 수량 = floor(cashAvailable / currentPrice). 0주면 판단 기록만 남기고 SKIP.
...
4. profit_rate >= target_profit_rate → SELL TARGET. 보유 종목이 현재 랭킹 1위여도 보류하지 않는다.
