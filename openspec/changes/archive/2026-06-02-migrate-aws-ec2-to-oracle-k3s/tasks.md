...
- [x] 1.4 Prepare OCIR credentials and document required GitHub Actions secrets/permissions and the cluster pull-secret approach (public repository or private OCIR auth token).
...
## 4. OCIR, ARM64 Build, and Manifest Migration
...
- [x] 4.2 Add OCIR login; push backend/frontend images to OCIR.
- [x] 4.3 Add `linux/arm64` (or multi-arch) build via buildx + QEMU or an ARM runner for backend and frontend.
- [x] 4.4 Update the GitOps tag-commit step to write OCIR `newName` values into `overlays/mvp/kustomization.yaml`.
- [x] 4.5 Update `overlays/mvp/kustomization.yaml` image `name`/`newName` to the OCIR coordinates.
- [x] 4.6 Remove ECR `imagePullSecrets` from backend/frontend Deployments (public repository) or replace with an OCIR pull secret (private repository).
...
- [x] 5.1 Recreate `infinite-buying-secrets` on the A1 cluster with the preserved `SECRET_ENCRYPTION_KEY`, `SESSION_SECRET`, and KIS values; create a OCIR pull secret if the package is private.
...
- [x] 7.5 Delete ECR repositories or apply lifecycle cleanup after confirming OCIR deployment is stable.
