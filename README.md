...
자동매매 화면의 **미국장 상승률 랭킹 전략** 탭은 미국 정규장 ET 10:00~16:00 동안 KIS 해외주식 상승률 랭킹을 30초마다 확인합니다. 보유 종목이 없으면 조건을 통과한 상승률 1위 종목을 사고, 보유 중이면 익절·손절·강제 청산 조건만 봅니다.
...
- 매수 후보: 현재가 1 USD 이상이고 당일 거래량 1,000만 주 이상인 종목만 고릅니다. 미국장은 한국장처럼 가격제한폭이 없어 별도 상승률 상한은 두지 않습니다.
- 매수 필터(단기 흐름 검사): 상위 후보의 당일 분봉(KIS 해외 분봉 `inquire-time-itemchartprice`, TR `HHDFS76950200`)을 보고 — 현재가 > VWAP, 현재가 > 최근 구간 시작가, 거래량 유지, 거래량 동반 장대 음봉 부재, 직전 고점을 밀리지 않음 — 을 확인합니다. 1위부터 차례로 최대 3개까지 보고 모두 떨어지면 그 평가는 건너뜁니다(블로우오프 고점 추격 방지).
- 매수: 보유 종목이 없을 때 필터를 통과한 가장 높은 상승률 종목을 고릅니다. 평가 시점의 USD 매수가능금액 전액으로 1주 단위 최대 수량을 계산합니다.
- 익절: 익절 기준에 닿으면 랭킹 순위와 관계없이 전량 매도합니다. 매도 후 손절·강제 청산·누적 목표 종료가 아니라면 다음 평가에서 다시 랭킹을 보고 재매수할 수 있습니다.
