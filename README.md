...
- 종목 선택: 오전·점심 모두 등락률 20% 이상 종목은 제외합니다(상한가까지 여유 확보). 상승률 상위 후보 순서대로 단기 흐름 필터를 한 번 더 거릅니다. 판단 로그와 진입 기록에는 재현 분석용 랭킹 스냅샷을 상위 30개까지 저장합니다.
- 매수 필터(단기 흐름 검사): 후보 종목의 당일 분봉(KIS `inquire-time-itemchartprice`)을 보고 다음을 확인합니다 — 현재가 > 시가, 현재가가 VWAP보다 0.1% 이상 위, 최근 완성봉 2개가 VWAP 위에서 마감, 직전 구간 대비 거래량 유지, 거래량을 동반한 장대 음봉 부재, 최근 고점 대비 1.2% 이상 밀리지 않음, 직전 고점의 99.5% 이상 유지. 모두 통과해야 매수합니다. 첫 후보가 떨어지면 차순위로 넘어가 최대 5개까지 본 다음 모두 떨어지면 그날(또는 그 진입 구간) 매수를 건너뜁니다.
