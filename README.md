...
- 진입: 오전은 9시부터 10분간, 점심 진입을 켜면 11시 20분부터 10분간 한국주식 등락률 상위 랭킹을 30초 간격으로 관찰합니다. 9시 10분·11시 30분의 첫 유효 평가에서 관찰 스냅샷과 현재 랭킹을 종합해 후보를 고르고, 신규 후보 선택과 주문 확인은 시작 후 3분 안에 끝냅니다. 후보가 없거나 모두 거절되면 `NO_CANDIDATE`로 해당 구간을 종결하며, 긴 진입창에서 조건이 우연히 한 번 통과할 때까지 새 후보를 반복 탐색하지 않습니다. 이미 접수된 주문의 체결 확인은 3분 뒤에도 계속합니다.
...
- 종목 선택: 먼저 각 스냅샷의 **원본 상승률 랭킹 상위 10위**를 자른 뒤, 오전은 등락률 15% 이상 20% 미만, 점심은 15% 미만인 종목만 남기고 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품을 제외합니다. 제외 후 11위 이하 종목으로 빈자리를 채우지 않습니다. 유효 관찰이 3회 미만이면 거래를 쉬고, 3회 이상이면 전체 스냅샷의 절반 이상 등장하고 최신 스냅샷에도 남은 종목만 후보로 인정합니다. 진입 전 관찰 스냅샷은 30일간 보관하고 판단 로그·진입 기록에는 상위 30개까지 저장합니다.
...
- 매수: 첫 필터 통과에서는 후보를 `SELECTED`로만 저장하고, 다음 평가에서 같은 후보의 최신 원본 순위·분봉·가격을 다시 확인한 뒤 주문합니다. 확인이 3분 안에 끝나지 않거나 후보가 원본 상위 10위 밖으로 밀리면 해당 구간을 종료합니다. 시장가가 아니라 **현재가 +0.4% 캡 지정가**로 보내며, 현재가가 분봉 신호가 또는 최초 선택가보다 0.7% 넘게 오르면 추격, 0.7% 넘게 내리면 모멘텀 무효로 판정합니다.
...
- 손절 유예: 매수 직후 손절 기준에 닿아도 항상 즉시 매도하지는 않습니다. 매수 후 10분 안, 손실이 10% 미만이면 하락의 성격을 보고 판단합니다 — 직전 분봉이 변동성(ATR) 대비 칼낙이거나, 하락봉 거래량이 상승봉의 2배 이상인 분출 매도이거나, 완성봉 기준 VWAP 지속 이탈·지지 붕괴가 확인되면 흔들기가 아닌 붕괴로 보고 곧바로 손절합니다. 셋 다 아니면 초기 흔들기로 보고 관찰합니다. 손실이 10% 이상으로 커지거나 보유 10분을 넘으면 기존 손절대로 전량 매도합니다.
