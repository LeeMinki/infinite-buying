...
| 한국 국장 상승률 랭킹 전략 | `KR_RANK_MOMENTUM` | 오전/점심 진입 시 등락률 상위 종목을 과열·상품군·단기 흐름 필터로 매수, 목표·손절·청산시각 매도 | 30초 |
...
- 평가 로그: 전략을 시작하면 즉시 시작 로그를 남기고, 이후 라오어 스케줄러가 최대 10분 간격으로 현재가, 보유 수량, 평균단가, 매수가능금액, 회차, 미체결 주문 수, 실주문 설정을 판단 로그에 남깁니다. 한국·미국 랭킹 전략은 기본 30초 간격으로 평가하되, 의미 있는 매수·매도·오류와 수동 평가 위주로 로그를 남깁니다.
...
- 주문 실패는 같은 멱등키 기준으로 제한된 횟수만 재시도합니다. 현재 라오어·한국·미국 랭킹 모두 실패 주문 재시도 한도는 5회이며, 안전 검증 실패나 미체결 주문 존재 시에는 새 주문을 만들지 않습니다.
...
- 진입: 오전은 9시부터 10분간, 점심 진입을 켜면 11시 20분부터 10분간 한국주식 등락률 상위 랭킹을 30초 간격으로 관찰합니다. 9시 10분·11시 30분부터 5분 동안 최근 관찰과 현재 랭킹을 갱신하며 후보를 찾습니다. 한 번 후보가 없거나 분봉·매수가능금액 조회가 실패했다고 구간을 끝내지 않고 제한 시간 안에서 다시 평가하되, 5분이 지나도록 후보가 없을 때만 `NO_CANDIDATE`로 종결합니다. 선택 후보는 다음 평가에서 한 번 더 확인하고, 선택 후 3분 안에 확인·제한 재시도가 끝나지 않으면 늦은 추격을 막기 위해 종료합니다.
- 진입 구간별 입력: 오전·점심 각각 매수 금액·목표 수익률·손절 기준을 따로 입력합니다. 점심 진입이 꺼져 있으면 오전 값만 입력합니다.
- 매수가능금액 전액 사용(선택): 켜면 매수 금액 입력 없이 진입 시점의 KIS 매수가능금액 전액을 한 종목에 투입합니다. 매도 후 잔액 변동이 다음 매수에 자동 반영됩니다.
- 종목 선택: 먼저 각 스냅샷의 **원본 상승률 랭킹 상위 10위**를 자른 뒤, 오전은 등락률 15% 이상 20% 미만, 점심은 15% 미만인 종목만 남기고 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품을 제외합니다. 제외 후 11위 이하 종목으로 빈자리를 채우지 않습니다. 최근 최대 12개 스냅샷 중 유효 관찰이 3회 미만이면 거래를 쉬고, 3회 이상이면 절반 이상 등장하고 최신 스냅샷에도 남은 종목만 후보로 인정합니다. 진입 전 관찰 스냅샷은 30일간 보관하고 판단 로그·진입 기록에는 상위 30개까지 저장합니다.
- 매수 필터(단기 흐름 검사): 후보 종목의 당일 분봉(KIS `inquire-time-itemchartprice`)을 보고 다음을 확인합니다 — 관찰 구간 거래대금 3억원 이상, 최근 3분 거래대금 3천만원 이상, 현재가 > 시가, 현재가가 VWAP보다 0.1% 이상 위, VWAP 대비 과열 아님(오전 8%, 점심 6% 이내), 점심은 최근 30분 VWAP 기준으로도 0.1% 이상 위이되 4% 넘게 높지 않음, 최근 3분 4%·8분 7% 이상 수직 급등 아님, 최근 완성봉 2개가 VWAP 위에서 마감, 직전 구간 대비 거래량 유지, 거래량을 동반한 장대 음봉 부재, 최근 고점 대비 1.2% 이상 밀리지 않음, 직전 고점의 99.5% 이상 유지. 종합 후보 5개 안에서 통과 후보를 점수로 고르고, 모두 떨어지면 그날(또는 그 진입 구간) 매수를 건너뜁니다.
- 매수: 첫 필터 통과에서는 후보를 `SELECTED`로만 저장하고, 다음 평가에서 같은 후보의 최신 원본 순위·분봉·가격을 다시 확인한 뒤 주문합니다. 확인이 3분 안에 끝나지 않거나 후보가 원본 상위 10위 밖으로 밀리면 해당 구간을 종료합니다. 시장가가 아니라 **현재가 +0.4% 캡 지정가**로 보내며, 현재가가 분봉 신호가 또는 최초 선택가보다 0.7% 넘게 오르면 추격, 0.7% 넘게 내리면 모멘텀 무효로 판정합니다. KIS가 주문번호 없이 거절을 명시하면 같은 후보를 다시 검증한 뒤 최대 5회 안에서 재시도합니다. 전송 결과가 모호한 timeout은 이중매수를 막기 위해 자동 재전송하지 않습니다. 주문번호가 있는 BUY가 90초 동안 전혀 체결되지 않으면 미체결·체결이력·잔고를 확인하고, 취소 완료가 확인된 경우에만 다음 평가에서 최신 신호로 재호가합니다.
- 주문·보유 격리: 실매수 직전에 같은 종목의 계좌 잔고가 이미 있으면 수동 보유분이나 다른 전략의 수량과 섞이지 않도록 그 진입을 건너뜁니다. 체결 뒤에도 이 전략의 BUY 주문에서 확인된 수량까지만 보유로 인정하고, 매도 수량은 그 BUY 수량에서 이미 체결된 SELL 수량을 뺀 범위로 제한합니다. 계좌의 같은 종목 전체 잔고를 전략 보유분으로 간주하지 않습니다.
...
- 연속 실행과 위험 집계: 실주문 손실 청산이 확인돼도 전략은 `RUNNING`을 유지하고, 오전 손실만으로 점심 진입을 자동 차단하지 않습니다. KIS 실현손익 동기화가 늦으면 BUY·SELL의 실제 체결가로만 보완하며, 체결가로도 손익을 정할 수 없는 청산은 주문 결정가로 추정하지 않고 손익 미확정으로 집계합니다. 같은 거래일 위험 청산이 2회 누적되면 그날 추가 신규 진입만 제한하고 다음 거래일에는 수동 재시작 없이 자동으로 다시 평가합니다. 분할 매도는 진입 1건으로 묶어 한 번만 세며, 이미 접수된 주문의 체결 확인과 기존 보유 청산은 위험 집계와 무관하게 계속합니다.
- 손절 유예와 하드 방어: 사용자가 5%보다 엄격한 손절 기준을 설정한 경우 매수 후 10분 안에는 칼낙·분출 매도·완성봉 기준 추세 붕괴 여부를 보고 초기 흔들기를 잠깐 유예할 수 있습니다. 하지만 손실이 **-5%에 닿으면 유예를 즉시 끝내고 시장가 청산을 시작**합니다. 급격한 갭 하락이나 거래정지에서는 실제 체결 손실을 -5% 이내로 보장할 수 없습니다.
...
- 매수: 보유 종목이 없을 때 필터를 통과한 가장 높은 상승률 종목을 고릅니다. 평가 시점의 USD 매수가능금액 전액으로 1주 단위 최대 수량을 계산합니다. KIS가 주문번호 없이 거절을 명시하면 같은 종목이 최신 상위 3개와 분봉 필터를 다시 통과하고, 최초 선택가 대비 ±2% 안일 때만 제한 재시도합니다. timeout처럼 접수 여부가 모호한 경우는 재전송하지 않습니다.
...
- 손절: 사용자 설정이 더 느슨해도 -5% 하드 방어선에 닿으면 전량 매도를 시작하고 그 미국 거래일에는 더 사지 않습니다. 갭과 지정가 체결 지연 때문에 실제 체결가를 -5% 이내로 보장하는 것은 아닙니다.
...
POST   /api/kr-rank/strategies/:id/sync-fills
GET    /api/kr-rank/strategies/:id/orders
GET    /api/kr-rank/strategies/:id/trade-history
POST   /api/kr-rank/strategies/:id/trade-history/:buyOrderId/replay
...
POST   /api/us-rank/strategies/:id/sync-fills
