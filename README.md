...
- 종목 선택: 오전·점심 모두 등락률 21% 이상 종목을 제외합니다. 우선주와 ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 매수 대상에서 제외합니다. 진입 전 10분 관찰 스냅샷에서 반복 출현, 평균 순위, 최신 순위를 종합해 상위 후보를 만들고 단기 흐름 필터를 한 번 더 거릅니다. 판단 로그와 진입 기록에는 재현 분석용 랭킹 스냅샷을 상위 30개까지 저장합니다.
...
- 손절 유예: 매수 직후 손절 기준에 닿아도 항상 즉시 매도하지는 않습니다. 매수 후 10분 안, 손실이 8% 미만이고 완성봉 기준 VWAP 지속 이탈·지지 붕괴가 확인되지 않으면 초기 흔들기로 보고 최대 6분까지 관찰합니다. 관찰 한도를 넘거나 구조 붕괴가 확인되면 기존 손절대로 전량 매도합니다.
