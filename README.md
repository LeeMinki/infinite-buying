...
- 진입: 오전은 9시부터 10분간, 점심 진입을 켜면 11시 20분부터 10분간 한국주식 등락률 상위 랭킹을 30초 간격으로 관찰합니다. 9시 10분·11시 30분 진입 시점에는 관찰 스냅샷과 현재 랭킹을 종합해 후보를 고릅니다. 각 진입은 하루 1회, 진입 구간당 1회만 매수하며 매도했더라도 같은 구간에서 다시 매수하지 않습니다. 점심 진입을 켜면 하루 두 번까지 매수합니다.
...
- 종목 선택: 오전은 등락률 21% 이상, 점심은 오전 급등 뒤 되돌림 위험을 줄이기 위해 15% 이상 종목을 제외합니다. 우선주와 ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 매수 대상에서 제외합니다. 진입 전 10분 관찰 스냅샷에서 반복 출현, 평균 순위, 최신 순위를 종합해 상위 후보를 만들고 단기 흐름 필터를 한 번 더 거릅니다. 관찰 스냅샷이 충분한데 반복 출현 후보가 없으면 억지로 한 종목을 고르지 않고 해당 진입을 건너뜁니다. 진입 전 관찰 스냅샷은 30일간 보관한 뒤 정리합니다. 판단 로그와 진입 기록에는 재현 분석용 랭킹 스냅샷을 상위 30개까지 저장합니다.
...
- 매수: 해당 진입 구간의 매수 금액 한도와 가용 현금 중 작은 값을 기준으로 정수 주 단위(한국주식)로 매수합니다. 시장가가 아니라 **현재가 +0.4% 캡 지정가**로 보내 정상 호가는 빠르게 체결하되 순간 급등 호가를 추격하지 않습니다. 실행 직전 현재가가 분봉 신호가보다 0.7% 넘게 올라 있으면 그 평가는 보류합니다.
