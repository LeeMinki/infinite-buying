...
- 종목 선택: 오전은 등락률 20% 이상, 점심은 16% 이상 종목을 제외합니다. 우선주와 ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 매수 대상에서 제외합니다. 상승률 상위 후보 순서대로 단기 흐름 필터를 한 번 더 거릅니다. 판단 로그와 진입 기록에는 재현 분석용 랭킹 스냅샷을 상위 30개까지 저장합니다.
- 매수 필터(단기 흐름 검사): 후보 종목의 당일 분봉(KIS `inquire-time-itemchartprice`)을 보고 다음을 확인합니다 — 현재가 > 시가, 현재가가 VWAP보다 0.1% 이상 위, VWAP 대비 과열 아님(오전 8%, 점심 6% 이내), 점심은 최근 30분 VWAP 기준으로도 0.1% 이상 위이되 4% 넘게 높지 않음, 최근 3분 4%·8분 7% 이상 수직 급등 아님, 최근 완성봉 2개가 VWAP 위에서 마감, 직전 구간 대비 거래량 유지, 거래량을 동반한 장대 음봉 부재, 최근 고점 대비 1.2% 이상 밀리지 않음, 직전 고점의 99.5% 이상 유지. 상위 5개 안에서 통과 후보를 점수로 고르고, 모두 떨어지면 그날(또는 그 진입 구간) 매수를 건너뜁니다.
