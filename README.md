...
- 전 재산 자동 매수(선택): 켜면 매수 금액 입력 없이 진입 시점의 KIS 매수가능금액 전액을 한 종목에 투입합니다. 매도 후 잔액 변동이 다음 매수에 자동 반영되어 사용자 개입 없이 매일 잔액 그대로 굴립니다.
...
- 실주문 실행 설정이 꺼져 있으면 랭킹 조회·종목 선택·판단·주문 예정 기록은 남기되 KIS 주문 API는 호출하지 않습니다. 진입 구간·매도 사유(목표 수익/손절/청산 시각)·실주문 여부를 구분해 기록합니다.

### 미국 국장 상승률 랭킹 전략

자동매매 화면의 **미국 국장 상승률 랭킹 전략** 탭은 미국 정규장 중 KIS 해외주식 상승률 랭킹을 1분 간격으로 조회해 한 종목씩 회전 매매합니다. 라오어·한국 랭킹 전략과 테이블과 화면은 분리되어 있고, 실주문 실행 설정과 KIS credential만 공유합니다.
...
- 종목 선택: 등락률 상한보다 낮은 종목 중 상승률이 가장 높은 종목을 선택합니다. 기본 상한은 20%입니다.
- 매수: 보유 종목이 없을 때만 새 종목을 고릅니다. 전 재산 자동 매수를 켜면 미국 종목 매수가능금액 전액을 기준으로 하고, 끄면 사용자가 입력한 고정 USD 금액을 한도로 씁니다. 미국 표준 주문은 1주 단위로 계산합니다.
- 매도: 보유 중에는 새 종목을 사지 않고 익절, 손절, 강제 청산만 확인합니다. 기본 익절은 +2%, 손절은 -5%입니다.
- 강제 청산: 기본 KST 04:30 이후에는 목표·손절 미도달이어도 전량 매도를 시도하고, 그 미국 거래일에는 신규 매수를 멈춥니다.
- 손절 잠금: 손절이 한 번 발생하면 같은 미국 거래일에는 다시 매수하지 않습니다. 다음 미국 거래일 첫 평가에서 잠금이 풀립니다.
- 주문 방식: KIS 미국 일반 매수 주문은 문서상 일반 시장가가 아니라 지정가·LOO·LOC·TWAP·VWAP 중심입니다. 앱은 현재가 지정가 주문으로 KIS 표준 해외 주문 경로를 사용합니다.
- 실주문 실행 설정이 꺼져 있으면 랭킹 조회·종목 선택·판단·주문 예정 기록은 남기되 KIS 주문 API는 호출하지 않습니다.
