...
- 종목 선택: 오전은 등락률 21% 이상, 점심은 오전 급등 뒤 되돌림 위험을 줄이기 위해 16% 이상 종목을 제외합니다. 우선주와 ETF·ETN·스팩·리츠·레버리지·인버스형 상품도 매수 대상에서 제외합니다. 진입 전 10분 관찰 스냅샷에서 반복 출현, 평균 순위, 최신 순위를 종합해 상위 후보를 만들고 단기 흐름 필터를 한 번 더 거릅니다. 모든 종목이 한 번씩만 스쳐가는 날에는 한 번 출현한 종목도 후보로 봅니다. 진입 전 관찰 스냅샷은 30일간 보관한 뒤 정리합니다. 판단 로그와 진입 기록에는 재현 분석용 랭킹 스냅샷을 상위 30개까지 저장합니다.
