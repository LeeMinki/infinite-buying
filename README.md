...
- 종목 선택: 먼저 각 스냅샷의 **원본 상승률 랭킹 상위 10위**를 자른 뒤 오전 21%·점심 15% 이상 종목과 우선주·ETF·ETN·스팩·리츠·레버리지·인버스형 상품을 제외합니다. 제외 후 11위 이하 종목으로 빈자리를 채우지 않습니다. 유효 관찰이 3회 미만이면 거래를 쉬고, 3회 이상이면 전체 스냅샷의 절반 이상 등장하고 최신 스냅샷에도 남은 종목만 후보로 인정합니다. 진입 전 관찰 스냅샷은 30일간 보관하고 판단 로그·진입 기록에는 상위 30개까지 저장합니다.
...
- 손실 회로 차단기: 실주문 손실 청산(`FILLED`)이 확인된 거래일에는 남은 신규 진입을 `SKIPPED`로 막습니다. KIS 실현손익 동기화가 늦으면 BUY·SELL 체결가로 우선 판정하고, 체결가로도 손익을 정할 수 없는 청산은 이익으로 간주하지 않고 확인이 끝날 때까지 신규 진입을 막습니다. 최근 청산 2건이 연속 손실 또는 손익 미확정이면 전략은 `RUNNING`을 유지하되 신규 진입을 잠급니다. 분할 매도는 진입 1건으로 묶어 한 번만 세며, 계좌 확인 후 전략을 명시적으로 중지하고 다시 시작하면 그 시각부터 다시 계산합니다.
