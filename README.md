...
- 이후에는 회차당 예산을 절반씩 나눕니다. 절반은 종가가 현재 평단가 이하일 때 매수하고, 나머지 절반은 큰수 매수로 계산합니다.
- 큰수 매수는 전일 종가에 큰수 매수 여유율을 더한 가격까지 허용합니다. 기본 10%이면 전일 종가의 110% 이하에서 큰수 매수가 체결된 것으로 계산합니다.
...
- 매수 조건: 보유 중에는 회차 예산을 절반씩 나눠 봅니다. 현재가가 평단가 이하이면 절반을 매수 대상으로 잡고, 현재가가 KIS에서 조회한 전일 종가 또는 기준가에 큰수 매수 여유율을 더한 가격 이하이면 나머지 절반도 매수 대상으로 잡습니다.
